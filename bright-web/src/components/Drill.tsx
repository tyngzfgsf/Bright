    ...
    <Section id="what" eyebrow={t("eyebrow")} title={t("title")} lede={t("lede")}>
      <ol className="grid gap-x-10 gap-y-12 md:grid-cols-3">
        {steps.map((step, i) => (
          <li key={step.title}>
            <Reveal delay={i * 0.07}>
              <div className="flex items-center gap-3 border-t border-line-strong pt-5">
        ...
        <div className="mt-16 rounded-[1.35rem] border border-line bg-raised px-6 py-6 sm:px-8">
          ...
          <ul className="mt-4 flex flex-wrap gap-2">
            {SCENARIOS.map((scenario) => (
              <li
                key={scenario.id}
                className="rounded-full border border-line px-3.5 py-1.5 text-[13.5px] text-ink-soft"
              >
                {scenario[locale]}
              </li>
            ))}
