...
import type { Copy } from "@/lib/app/copy";
import { EASE, GLIDE, PRESS } from "@/lib/motion";
import { useAuth } from "@/lib/app/auth";
import { averageScore, groupSessions, type GroupKey, type Session } from "@/lib/app/sessions";
...
export default function Sidebar({
  ...
  onOpenSettings: () => void;
  onOpenAccount: () => void;
  /** Back to the rest of the site — same URL, the app just steps aside. */
  onHome: () => void;
    ...
    <aside className="flex h-full w-[17.5rem] flex-col border-r border-line-subtle bg-sunken">
      <div className="flex items-center justify-between px-3 pb-1 pt-3">
        <button
          ...
          title={t.backToSite}
          className="flex items-center gap-2 rounded-[0.65rem] py-1 pl-2 pr-2.5 transition-colors duration-200 hover:bg-raised"
        >
